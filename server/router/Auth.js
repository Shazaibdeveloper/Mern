...
router.post("/contact", auth, async function (req, res) {
  try {
    ...
    if (!name || !work || !email || !message) {
      return res.json({ message: "Plz Fill the data" });
    }

    const databaseuser = await User.findOne({ _id: req.userID });
    if (databaseuser) {
      const usermsg = await databaseuser.addMessage(name, email, work, message);
